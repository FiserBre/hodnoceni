...
async function sendBadReviewEmail(review) {
  if (!transporter) {
    console.log('[EMAIL SKIPPED - no SMTP config] Bad review from:', review.email, 'Stars:', review.overall_stars);
    return;
  }
...
app.post('/api/reviews', async (req, res) => {
  ...
  try {
    ...
    if (flagged) {
      await sendBadReviewEmail(review).catch(e => console.error('Email error:', e.message));
    }
...
app.get('/api/stats', async (req, res) => {
  ...
  try {
    if (pool) {
      ...
      const avg = total ? (memReviews.reduce((a,r)=>a+r.overall_stars,0)/total).toFixed(2) : 0;
