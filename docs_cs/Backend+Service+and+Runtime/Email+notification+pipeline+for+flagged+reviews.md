# Pipeline pro e-mailové oznámení o nahlášených recenzích

## Přehled

Tato runtime cesta transformuje odeslanou recenzi na volitelné SMTP upozornění v případě, že je hodnocení negativní. V souboru server.js dojde k validaci dat (payloadu) recenze, přičemž stav „nahlášeno“ (flagged) je odvozen z podmínky overall_stars < 4. Následně je recenze vyhodnocena jako způsobilá k odeslání e-mailového oznámení na adresu nakonfigurovanou v proměnné NOTIFY_EMAIL.

Tok oznámení je z produktového hlediska záměrně navržen jako neblokující: pokud není SMTP nakonfigurováno, pomocná funkce (helper) zaprotokoluje vynechané upozornění do logu a ukončí se. To zajišťuje, že cesta pro odeslání recenze zůstává funkční i v případě, že doručování e-mailů není dostupné.
