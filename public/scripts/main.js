/* Main page interactions: star rating, submission, modal */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (submitBtn) {
    submitBtn.addEventListener('click', async () => {
      ...
      if (!overallVal) {
        if (err) err.textContent = 'Prosím vyberte celkové hodnocení hvězdičkami.';
        return;
      }
      ...
      submitBtn.innerHTML = '<span class="spinner"></span>Odesílám...';
      ...
      try {
        ...
        if (err) err.textContent = 'Chyba při odesílání: ' + (e.message || 'Zkuste to prosím znovu.');
