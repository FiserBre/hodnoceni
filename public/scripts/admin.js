...
function stars(n, max = 5) {
  ...
  return (
    "★".repeat(n) + '<span class="empty">' + "★".repeat(max - n) + "</span>"
  );
}
...
async function loadStats() {
  try {
    ...
    document.getElementById("sAvg").textContent = d.avg_stars
      ? "★" + parseFloat(d.avg_stars).toFixed(1)
      : "—";
...
async function loadReviews() {
  ...
  try {
    ...
    document.getElementById("loadingState").innerHTML =
      '<p style="color:var(--red)">Chyba: ' + e.message + "</p>";
  }
}
...
function renderReviews() {
  ...
  if (currentFilter === "positive")
    reviews = reviews.filter((r) => r.overall_stars >= 4);
